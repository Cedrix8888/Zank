...
export default function Canvas({ images }) {
    // imageUrl is now expected to be a dict: {layerName: imagePath}
    ...
    useEffect(() => {
        if (imageUrl && typeof imageUrl === 'object' && !Array.isArray(imageUrl)) {
            // imageUrl is a dict: {layerName: imagePath}
            const dict = {};
            Object.entries(imageUrl).forEach(([key, url]) => {
                dict[key] = `${API_CONFIG.baseUrl}${url}`;
            });
            setFullImageUrls(dict);
        } else if (Array.isArray(imageUrl)) {
            // fallback: treat as array, use index as key
            const dict = {};
            imageUrl.forEach((url, idx) => {
                dict[idx] = `${API_CONFIG.baseUrl}${url}`;
            });
            setFullImageUrls(dict);
        } else if (imageUrl) {
            setFullImageUrls({ 0: `${API_CONFIG.baseUrl}${imageUrl}` });
        } else {
            setFullImageUrls({});
        }
    }, [imageUrl]);
    ...
    return (
        <div className="top-[36.5px] fixed right-0 w-290 h-full bg-gray-100 shadow-lg">
            ...
            <div className="w-full h-full flex justify-center items-center p-8 pb-16" style={{ position: 'relative' }}>
                {Object.entries(fullImageUrls).map(([layer, url], idx) => (
                    <img
                        key={layer}
                        src={url}
                        alt={`Layer ${layer}`}
