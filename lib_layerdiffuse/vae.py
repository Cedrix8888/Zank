# Encoder and decoder realizing the conversion between RGBA images and the latent space.
# Basic process: 
#   1.encoding:
#       TransparentVAEEncoder receives RGBA images.
#       Encode with SD_VAE to obtain the basic latent distribution.
#       Generate offsets using the LatentTransparencyOffsetEncoder, adjust the latent distribution, and sample to obtain the final latent vector.
#   2.decoding:
#       The TransparentVAEDecoder receives the latent vector and first decodes it using SD_VAE to obtain the RGB image.
#       The UNet1024 combines the RGB image and the latent vector, performs multiple predictions through data augmentation (flipping + rotation), and takes the median value to obtain a stable RGBA image.
#       When visualizing, the transparent areas display a checkerboard background.
...
def zero_module(module):
    ...
    # Avoid the impact of initial parameters on the output results.
